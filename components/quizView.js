...
class QuizView extends Component {
  ...
  render() {
    ...
    const deck = decks[this.props.navigation.state.params.deckTitle];
    const showQuestion =
      this.state.count < deck.questions.length ? true : false;
    const currentQuestion = deck.questions[this.state.count];

    if (deck.questions.length === 0) {
      return (
        <Card>
          <Text>No cards are left in this deck.</Text>
        </Card>
      );
    }
    return (
      <View style={styles.container}>
        {showQuestion ? (
          <Card>
            <Text>
              {this.state.count} / {deck.questions.length}
            </Text>
          ...
          <Card>
            <Text style={styles.text}>
              Correct Answers: {this.state.correctAnswers} out of{" "}
